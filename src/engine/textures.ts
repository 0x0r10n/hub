...
export function makeFloorTile(ambience: Ambience, accent: string, variant: number): HTMLCanvasElement {
  const canvas = makeCanvas(TILE_PX, TILE_PX);
  const ctx = canvas.getContext("2d")!;
  const rand = mulberry32(ambience.length * 97 + variant * 13);
  const base = mix(VOID_950, accent, 0.06 + variant * 0.01);
  ctx.fillStyle = base;
  ...
  for (let gy = 0; gy < grid; gy++) {
    for (let gx = 0; gx < grid; gx++) {
      if (rand() < 0.22) {
        ctx.fillStyle = mix(base, VOID_950, 0.2 + rand() * 0.25);
        ctx.fillRect(gx * cell, gy * cell, cell, cell);
      }
    }
  }

  ctx.strokeStyle = mix(base, VOID_950, 0.45);
  ctx.lineWidth = 1;
  ctx.strokeRect(0.5, 0.5, TILE_PX - 1, TILE_PX - 1);

  switch (ambience) {
    case "water": {
      ctx.fillStyle = mix(base, accent, 0.35);
      for (let y = 4; y < TILE_PX; y += 8) {
        const off = variant % 2 === 0 ? 0 : 4;
        ctx.fillRect((0 + off) % TILE_PX, y, 10, 2);
        ctx.fillRect((16 + off) % TILE_PX, y + 4, 8, 2);
      }
      break;
    }
    case "neon": {
      ctx.strokeStyle = mix(base, accent, 0.5);
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(0, TILE_PX / 2);
    ...
    case "circuit": {
      ctx.strokeStyle = mix(base, accent, 0.55);
      ctx.lineWidth = 1;
      ctx.strokeRect(6, 6, TILE_PX - 12, TILE_PX - 12);
      speckle(ctx, rand, 3, mix(base, accent, 0.7), 2, TILE_PX, TILE_PX);
      break;
    }
    case "static": {
      speckle(ctx, rand, 10, mix(base, "#000000", 0.3), 2, TILE_PX, TILE_PX);
      speckle(ctx, rand, 4, mix(base, accent, 0.3), 2, TILE_PX, TILE_PX);
      break;
    }
    case "bloom": {
      speckle(ctx, rand, 6, mix(base, accent, 0.4), 3, TILE_PX, TILE_PX);
      break;
    }
    case "data": {
      ctx.fillStyle = mix(base, accent, 0.35);
      for (let y = 2; y < TILE_PX; y += 6) {
        for (let x = (y % 12); x < TILE_PX; x += 12) ctx.fillRect(x, y, 4, 1);
      }
      break;
    }
    case "sky": {
      ctx.strokeStyle = mix(base, "#000000", 0.3);
...
export function makeMetaFloorTile(ambience: Ambience, accent: string, grid = 4): HTMLCanvasElement {
  const variants = [makeFloorTile(ambience, accent, 0), makeFloorTile(ambience, accent, 1), makeFloorTile(ambience, accent, 2)];
  const size = TILE_PX * grid;
  const canvas = makeCanvas(size, size);
  const ctx = canvas.getContext("2d")!;
  const rand = mulberry32(accent.length * 31 + ambience.length * 17);
  for (let gy = 0; gy < grid; gy++) {
    for (let gx = 0; gx < grid; gx++) {
...
export function makeBuildingTexture(w: number, h: number, accent: string, state: RoomVisualState, seed: number, zoneId?: ZoneId): HTMLCanvasElement {
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      if (rand() < 0.15) {
        ctx.fillStyle = shade(wallBase, -0.12 - rand() * 0.12);
  ...
  // windows -- a visible frame regardless of state, glowing when the room is occupied
  const winCount = Math.max(2, Math.floor(cols / 2));
  const winW = Math.min(10, w / (winCount * 2.4));
  const winH = winW * 1.2;
  const winY = roofH + (h - roofH) * 0.22;
...
function drawPatternFrame(
  ctx: CanvasRenderingContext2D,
  pattern: string[],
  opts: { originX: number; originY: number; cellW: number; cellH: number; accent: string; direction: Direction; animPhase: number; walking: boolean; float: boolean },
) {
...
export function buildCharacterSheet(variant: SpriteVariant, accent: string): CharSheetInfo {
...
export function makePortraitCanvas(variant: SpriteVariant, accent: string, size = 48): HTMLCanvasElement {
  const canvas = makeCanvas(size, size);
  const ctx = canvas.getContext("2d")!;
  const pattern = BASE_PATTERNS[variant];
...
export function getPortraitDataURL(variant: SpriteVariant, accent: string, size = 48): string {
  const key = `${variant}:${accent}:${size}`;
  const cached = portraitCache.get(key);
  if (cached) return cached;
  const url = makePortraitCanvas(variant, accent, size).toDataURL();
